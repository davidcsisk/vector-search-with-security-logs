...
def generate_malicious_log_entry(entry_id):
    ip = generate_ip()
    timestamp = generate_timestamp().strftime("%d/%b/%Y:%H:%M:%S %z")
...
# Generate 1000 log entries with 950 normal and 50 malicious and write to CSV
with open("proxy_logs_mixed.csv", "w", newline='') as csvfile:
    ...
    for entry_id in range(951, 1001):
        log_entry = generate_malicious_log_entry(entry_id)
        writer.writerow({'ID': log_entry[0], 'IP Address': log_entry[1], 'Timestamp': log_entry[2], 'Log Entry': log_entry[3]})
